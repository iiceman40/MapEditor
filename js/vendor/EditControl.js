				...
				var EditControl = (function () {
					function EditControl(mesh, camera, canvas, scale) {
						...
						this.scene.registerBeforeRender(function () {
							return _this.renderLoopProcess();
						});
					}
					...
					EditControl.prototype.detach = function () {
						...
						this.canvas.removeEventListener("pointerdown", function (evt) {
							return _this.onPointerDown(evt);
						}, false);
						this.canvas.removeEventListener("pointerup", function (evt) {
							return _this.onPointerUp(evt);
						}, false);
						this.canvas.removeEventListener("pointermove", function (evt) {
							return _this.onPointerMove(evt);
						}, false);
						this.scene.unregisterBeforeRender(function () {
							return _this.renderLoopProcess();
						});
					};
					EditControl.prototype.onPointerDown = function (evt) {
						...
						if ((pickResult.hit)) {
							...
							window.setTimeout((function (cam, can) {
								return _this.detachControl(cam, can);
